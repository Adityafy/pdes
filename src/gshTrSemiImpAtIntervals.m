function [psi, omz, zeta, u, v] = gshTrSemiImpAtIntervals(p)
% [psi, omz, zeta, u, v] = gshTrDynSemiImpAtIntervals(p)
% The time integration function that yields psi, omega_z, zeta as matrices 
% evolved from initial states through transient time units.
addpath('../src/');

dt = p.dt_tr; % time step size

totimeu = p.trtimeu;
tmax = totimeu/dt; % total time steps

intervals = p.TrIntervals;
% if tmax < intervals
...
[matdivpsi, matdivomz] = impMatGSH(p,p.dt_tr);
toc
