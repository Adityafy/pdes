%% Test script for Swift-Hohenberg 1D with Pseudospectral v2
...
x2 = 2*pi*linspace(-Lx/2,Lx/2,Nx+1)';
x = x2(1:Nx);
dx = x(2) - x(1);
...
figure;
% contourf(u','LevelStep',0.1,'EdgeColor','none'); 
imagesc(x,1:nmax,uetd1exp');
axis square;
colormap jet; colorbar;
set(gca,'YDir','normal');
xlabel('$i$','Interpreter','latex','FontSize',30);
ylabel('$n$','Interpreter','latex','FontSize',30,'Rotation',0);
...
%% wavenumber
figure; hold on;
plot(kx,real(fft(uetd1exp(:,end))),'-o','LineWidth',1);
xlim([min(kx) max(kx)]);
